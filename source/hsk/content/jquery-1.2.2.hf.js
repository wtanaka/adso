...
load_new_question = function() {
      document.getElementById("review_incorrect").innerHTML = "";
      document.getElementById("review_incorrect").innerHTML += '<div style="font-size:0.8em;color:#9c1010;font-weight:bold;margin-bottom:10px;margin-left:5px;">Review:</div>';
      document.getElementById("review_incorrect").innerHTML += '<img src="/img/loader-big-circle.gif" alt="loading...." />';
      $.post("/words/loadSingleQuestion?previous="+current_question,{
      },function(txt){
        document.getElementById("review_incorrect").innerHTML = "";
	if (txt != "") {
          document.getElementById("review_incorrect").innerHTML += '<div style="font-size:0.8em;color:#9c1010;font-weight:bold;margin-bottom:10px;margin-left:5px;">Review:</div>';
          document.getElementById("review_incorrect").innerHTML += txt;
	  var temp = txt.split("lesson_question_");
	  current_question = temp[1].split("\"")[0];
	}
      });
}
